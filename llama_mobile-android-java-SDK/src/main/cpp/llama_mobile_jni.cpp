...
// Get TTS type
JNIEXPORT jobject JNICALL Java_com_llamamobile_LlamaMobile_getTTSType(
    JNIEnv *env, jobject thiz, jlong contextHandle) {
    
    if (contextHandle == 0) {
        // Return UNKNOWN
        jclass ttsTypeClass = env->FindClass("com/llamamobile/LlamaMobile$TTSModelType");
        jfieldID unknownField = env->GetStaticFieldID(ttsTypeClass, "UNKNOWN", "Lcom/llamamobile/LlamaMobile$TTSModelType;");
        return env->GetStaticObjectField(ttsTypeClass, unknownField);
    }
    
    int ttsType = llama_mobile_get_tts_type_c(
        reinterpret_cast<llama_mobile_context_handle_t>(contextHandle)
    );
    
    jclass ttsTypeClass = env->FindClass("com/llamamobile/LlamaMobile$TTSModelType");
    jmethodID fromIntMethod = env->GetStaticMethodID(ttsTypeClass, "fromInt", "(I)Lcom/llamamobile/LlamaMobile$TTSModelType;");
    
    return env->CallStaticObjectMethod(ttsTypeClass, fromIntMethod, ttsType);
}
