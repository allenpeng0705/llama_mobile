...
static void log_mel_spectrogram_worker_thread(int ith, const float * hann, const std::vector<float> & samples,
    ...
    LM_GGML_ASSERT(g_cache.sin_vals.size() == g_cache.cos_vals.size());
...
bool mtmd_audio_preprocessor_whisper::preprocess(
    ...
    LM_GGML_ASSERT(!g_cache.sin_vals.empty());
    LM_GGML_ASSERT(!g_cache.cos_vals.empty());
    LM_GGML_ASSERT(!g_cache.filters.data.empty());
