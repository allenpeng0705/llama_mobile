...
static struct lm_ggml_backend_reg    g_lm_ggml_metal_reg;
static struct lm_ggml_backend_device g_lm_ggml_metal_device;
...
static void lm_ggml_backend_metal_buffer_shared_memset_tensor(lm_ggml_backend_buffer_t buffer, struct lm_ggml_tensor * tensor, uint8_t value, size_t offset, size_t size) {
    lm_ggml_metal_buffer_t ctx = (lm_ggml_metal_buffer_t)buffer->context;
...
static void lm_ggml_backend_metal_buffer_shared_set_tensor(lm_ggml_backend_buffer_t buffer, struct lm_ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    lm_ggml_metal_buffer_t ctx = (lm_ggml_metal_buffer_t)buffer->context;
...
static void lm_ggml_backend_metal_buffer_shared_get_tensor(lm_ggml_backend_buffer_t buffer, const struct lm_ggml_tensor * tensor, void * data, size_t offset, size_t size) {
    lm_ggml_metal_buffer_t ctx = (lm_ggml_metal_buffer_t)buffer->context;
...
static bool lm_ggml_backend_metal_buffer_shared_cpy_tensor(lm_ggml_backend_buffer_t buffer, const struct lm_ggml_tensor * src, struct lm_ggml_tensor * dst) {
    lm_ggml_metal_buffer_t ctx = (lm_ggml_metal_buffer_t)buffer->context;
...
static struct lm_ggml_backend_buffer_i lm_ggml_backend_metal_buffer_shared_i = {
...
static void lm_ggml_backend_metal_buffer_private_memset_tensor(lm_ggml_backend_buffer_t buffer, struct lm_ggml_tensor * tensor, uint8_t value, size_t offset, size_t size) {
    lm_ggml_metal_buffer_t ctx = (lm_ggml_metal_buffer_t)buffer->context;
...
static void lm_ggml_backend_metal_buffer_private_set_tensor(lm_ggml_backend_buffer_t buffer, struct lm_ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    lm_ggml_metal_buffer_t ctx = (lm_ggml_metal_buffer_t)buffer->context;
...
static void lm_ggml_backend_metal_buffer_private_get_tensor(lm_ggml_backend_buffer_t buffer, const struct lm_ggml_tensor * tensor, void * data, size_t offset, size_t size) {
    lm_ggml_metal_buffer_t ctx = (lm_ggml_metal_buffer_t)buffer->context;
...
static bool lm_ggml_backend_metal_buffer_private_cpy_tensor(lm_ggml_backend_buffer_t buffer, const struct lm_ggml_tensor * src, struct lm_ggml_tensor * dst) {
    lm_ggml_metal_buffer_t ctx = (lm_ggml_metal_buffer_t)buffer->context;
...
static struct lm_ggml_backend_buffer_i lm_ggml_backend_metal_buffer_private_i = {
...
static lm_ggml_backend_buffer_t lm_ggml_backend_metal_buffer_type_alloc_buffer(lm_ggml_backend_buffer_type_t buft, size_t size, bool shared) {
    ...
    return lm_ggml_backend_buffer_init(buft, lm_ggml_metal_buffer_is_shared(res)
        ? lm_ggml_backend_metal_buffer_shared_i
        : lm_ggml_backend_metal_buffer_private_i, res, size);
}

static size_t lm_ggml_backend_metal_buffer_type_get_alloc_size(lm_ggml_backend_buffer_type_t buft, const struct lm_ggml_tensor * tensor) {
...
static size_t lm_ggml_backend_metal_buffer_type_shared_get_alloc_size(lm_ggml_backend_buffer_type_t buft, const struct lm_ggml_tensor * tensor) {
...
static size_t lm_ggml_backend_metal_buffer_type_private_get_alloc_size(lm_ggml_backend_buffer_type_t buft, const struct lm_ggml_tensor * tensor) {
    return lm_ggml_backend_metal_buffer_type_get_alloc_size(buft, tensor);
}
...
static size_t lm_ggml_backend_metal_buffer_type_mapped_get_alloc_size(lm_ggml_backend_buffer_type_t buft, const struct lm_ggml_tensor * tensor) {
    return lm_ggml_backend_metal_buffer_type_get_alloc_size(buft, tensor);
}
...
static lm_ggml_backend_buffer_type_t lm_ggml_backend_metal_buffer_type_mapped(void) {
    ...
    static struct lm_ggml_backend_buffer_type lm_ggml_backend_buffer_type_mapped_metal = {
        /* .iface = */ {
...
static void lm_ggml_backend_metal_set_tensor_async(lm_ggml_backend_t backend, struct lm_ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    lm_ggml_metal_t ctx = (lm_ggml_metal_t)backend->context;

    lm_ggml_metal_set_tensor_async(ctx, tensor, data, offset, size);
}

static void lm_ggml_backend_metal_get_tensor_async(lm_ggml_backend_t backend, const struct lm_ggml_tensor * tensor, void * data, size_t offset, size_t size) {
    lm_ggml_metal_t ctx = (lm_ggml_metal_t)backend->context;

    lm_ggml_metal_get_tensor_async(ctx, tensor, data, offset, size);
}

static bool lm_ggml_backend_metal_cpy_tensor_async(lm_ggml_backend_t backend_src, lm_ggml_backend_t backend_dst, const struct lm_ggml_tensor * src, struct lm_ggml_tensor * dst) {
    return false;
...
static void lm_ggml_backend_metal_graph_optimize(lm_ggml_backend_t backend, struct lm_ggml_cgraph * cgraph) {
    lm_ggml_metal_t ctx = (lm_ggml_metal_t)backend->context;
...
lm_ggml_backend_t lm_ggml_backend_metal_init(void) {
    ...
    lm_ggml_backend_t backend = (lm_ggml_backend_t) malloc(sizeof(struct lm_ggml_backend));
    if (backend == NULL) {
        lm_ggml_metal_free(ctx);
        return NULL;
    }

    memset(backend, 0, sizeof(struct lm_ggml_backend));
    backend->guid = lm_ggml_backend_metal_guid();
    backend->iface = lm_ggml_backend_metal_i;
    backend->device = dev;
    backend->context = ctx;
...
static lm_ggml_backend_buffer_type_t lm_ggml_backend_metal_device_get_buffer_type(lm_ggml_backend_dev_t dev) {
    ...
    const struct lm_ggml_metal_device_props * props_dev = lm_ggml_metal_device_get_props(ctx_dev);
...
static int64_t get_op_batch_size(const struct lm_ggml_tensor * op) {
    switch (op->op) {
...
lm_ggml_backend_reg_t lm_ggml_backend_metal_reg(void) {
    memset(&g_lm_ggml_metal_reg, 0, sizeof(g_lm_ggml_metal_reg));
    g_lm_ggml_metal_reg.api_version = LM_GGML_BACKEND_API_VERSION;
    g_lm_ggml_metal_reg.iface = lm_ggml_backend_metal_reg_i;
    g_lm_ggml_metal_reg.context = NULL;

    memset(&g_lm_ggml_metal_device, 0, sizeof(g_lm_ggml_metal_device));
    g_lm_ggml_metal_device.iface = lm_ggml_backend_metal_device_i;
    g_lm_ggml_metal_device.reg = &g_lm_ggml_metal_reg;
    g_lm_ggml_metal_device.context = lm_ggml_metal_device_get();
