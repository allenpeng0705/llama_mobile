...
utf8_parse_result parse_utf8_codepoint(const std::string & input, size_t pos) {
    ...
    uint8_t byte0 = static_cast<uint8_t>(input[pos]);

    // 1-byte UTF-8 sequence
    if (!(byte0 & 0x80)) {
        result.status = utf8_parse_result::VALID;
        result.codepoint = byte0;
        result.bytes_consumed = 1;
        return result;
    }

    // Invalid UTF-8: continuation byte as first byte
    if (!(byte0 & 0x40)) {
        result.status = utf8_parse_result::INVALID;
        result.bytes_consumed = 1;
        return result;
    ...
    if (!(byte0 & 0x20)) {
        ...
        uint8_t byte1 = static_cast<uint8_t>(input[pos + 1]);
        if (!((byte1 & 0xc0) == 0x80)) {
            result.status = utf8_parse_result::INVALID;
            result.bytes_consumed = 1;
            return result;
        ...
        result.status = utf8_parse_result::VALID;
        result.codepoint = ((byte0 & 0x1f) << 6) | (byte1 & 0x3f);
    ...
    if (!(byte0 & 0x10)) {
        ...
        uint8_t byte1 = static_cast<uint8_t>(input[pos + 1]);
        uint8_t byte2 = static_cast<uint8_t>(input[pos + 2]);
        ...
        result.status = utf8_parse_result::VALID;
        result.codepoint = ((byte0 & 0x0f) << 12) | ((byte1 & 0x3f) << 6) | (byte2 & 0x3f);
    ...
    if (!(byte0 & 0x08)) {
        ...
        uint8_t byte1 = static_cast<uint8_t>(input[pos + 1]);
        uint8_t byte2 = static_cast<uint8_t>(input[pos + 2]);
        uint8_t byte3 = static_cast<uint8_t>(input[pos + 3]);
        ...
        result.status = utf8_parse_result::VALID;
        result.codepoint = ((byte0 & 0x07) << 18) | ((byte1 & 0x3f) << 12) | ((byte2 & 0x3f) << 6) | (byte3 & 0x3f);
        result.bytes_consumed = 4;
        return result;
    }

    // Invalid UTF-8: 5 or more bytes
    result.status = utf8_parse_result::INVALID;
    result.bytes_consumed = 1;
    return result;
}
