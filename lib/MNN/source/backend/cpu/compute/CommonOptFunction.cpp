...
void MNNUnpackC2Common(T* dst, const T* src, size_t area, size_t depth, int* areaOffset, int pack = 1) {
    size_t depthC2     = depth / 2;
    size_t depthRemain = depthC2 * 2;
    size_t remain      = depth - depthRemain;
    int z, x, y;
    const T* srcChannel[2];
    const T* srcOffset = src;
    for(z = 0; z < depthC2; ++z) {
...
void MNNCoreFunctionInit() {
    ...
    gCoreFunction->MNNAdjustOptimalSparseKernel = nullptr;
...
#else
// Scalar implementation for MNNPackC4Origin when NEON is disabled
void MNNPackC4Origin(float* dst, const float* src, size_t area, size_t depth, int areaOffset) {
    const int pack = 4;
    const int depthC4 = (depth + pack - 1) / pack;
    
    for (size_t d = 0; d < depthC4; ++d) {
        for (size_t a = 0; a < area; ++a) {
            float* dstPtr = dst + d * area + a;
            const float* srcPtr = src + (d * pack) * areaOffset + a;
            
            for (int p = 0; p < pack; ++p) {
                if (d * pack + p < depth) {
                    *dstPtr = srcPtr[p * areaOffset];
                } else {
                    *dstPtr = 0.0f;
                }
                dstPtr += area * depthC4;
