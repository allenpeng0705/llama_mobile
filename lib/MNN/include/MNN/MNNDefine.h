...
#include <syslog.h>
// Undefine macros that conflict with llama_mobile_unified.h
#undef LOG_ERROR
#undef LOG_WARNING
#undef LOG_INFO
#define MNN_PRINT(format, ...) syslog(4, format, ##__VA_ARGS__); fprintf(stderr, format, ##__VA_ARGS__)
#define MNN_ERROR(format, ...) syslog(4, format, ##__VA_ARGS__); fprintf(stderr, format, ##__VA_ARGS__)
#else
