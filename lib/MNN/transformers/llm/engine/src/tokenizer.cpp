...
Sentencepiece::EncodeResult Sentencepiece::bpe_encode(string_view_ normalized, float alpha) {
    ...
    while (!normalized.empty()) {
        ...
        size_t mblen = std::min(normalized.size(), one_char_len(normalized.data()));
    ...
    for (size_t i = 1; i < symbols.size(); ++i) {
        MaybeAddNewSymbolPair(static_cast<int>(i - 1), static_cast<int>(i));
    }
...
void HuggingfaceTokenizer::bpe(const std::wstring& token, const BPERanks& bpe_ranks, std::vector<std::wstring>* result) {
    std::set<size_t> merged;  // records indices in pairs that were merged.
    auto _left = [](size_t i, std::set<size_t>& merged) {
        for (size_t j = i - 1; j != static_cast<size_t>(-1); j--) {
            if (merged.find(j) == merged.end()) return j;
        }
        return static_cast<size_t>(-1);
    };
    auto _right = [](size_t i, size_t cap, std::set<size_t>& merged) {
    ...
    while (true) {
        ...
        size_t to_merge = static_cast<size_t>(-1);  // indices into pairs.
        ...
        if (l != static_cast<size_t>(-1)) pairs[l].second = merge_into;
