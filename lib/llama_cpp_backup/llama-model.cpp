...
static bool weight_buft_supported(const llama_hparams & hparams, lm_ggml_tensor * w, lm_ggml_op op, lm_ggml_backend_buffer_type_t buft, lm_ggml_backend_dev_t dev) {
    ...
    switch (op) {
        case LM_GGML_OP_GET_ROWS:
            {
                lm_ggml_tensor * b = lm_ggml_new_tensor_1d(ctx, GGML_TYPE_I32, 512);
                op_tensor = lm_ggml_get_rows(ctx, w, b);
            } break;
        case LM_GGML_OP_MUL_MAT:
            {
                lm_ggml_tensor * b = lm_ggml_new_tensor_4d(ctx, GGML_TYPE_F32, w->ne[0], 512, w->ne[2], w->ne[3]);
                op_tensor = lm_ggml_mul_mat(ctx, w, b);
            } break;
        case LM_GGML_OP_MUL_MAT_ID:
            {
                int n_expert_used = hparams.n_expert_used;
                lm_ggml_tensor * b = lm_ggml_new_tensor_3d(ctx, GGML_TYPE_F32, w->ne[0], n_expert_used, 512);
                lm_ggml_tensor * ids = lm_ggml_new_tensor_2d(ctx, GGML_TYPE_I32, n_expert_used, 512);
                op_tensor = lm_ggml_mul_mat_id(ctx, w, b, ids);
            } break;
        case LM_GGML_OP_ADD:
            {
                lm_ggml_tensor * a = lm_ggml_new_tensor_4d(ctx, GGML_TYPE_F32, w->ne[0], w->ne[1], w->ne[2], w->ne[3]);
                op_tensor = lm_ggml_add(ctx, a, w);
            } break;
        case LM_GGML_OP_ADD_ID:
            {
                int n_expert_used = hparams.n_expert_used;
                lm_ggml_tensor * a = lm_ggml_new_tensor_3d(ctx, GGML_TYPE_F32, w->ne[0], n_expert_used, 512);
                lm_ggml_tensor * c = lm_ggml_new_tensor_2d(ctx, GGML_TYPE_I32, n_expert_used, 512);
        ...
        case LM_GGML_OP_DIV:
            {
                lm_ggml_tensor * a = lm_ggml_new_tensor_1d(ctx, GGML_TYPE_F32, w->ne[0]);
        ...
        case LM_GGML_OP_ROPE:
            {
                ...
                lm_ggml_tensor * a = lm_ggml_new_tensor_3d(ctx, GGML_TYPE_F32, n_embd_head, n_head, 512);
                lm_ggml_tensor * b = lm_ggml_new_tensor_1d(ctx, GGML_TYPE_I32, 512);
        ...
        case LM_GGML_OP_SSM_CONV:
            {
                ...
                lm_ggml_tensor * conv_x = lm_ggml_new_tensor_3d(ctx, GGML_TYPE_F32, w->ne[0] - 1 + n_seq_tokens, w->ne[1], n_seqs);
        ...
        case LM_GGML_OP_SSM_SCAN:
            {
                ...
                const int64_t n_seq_tokens = 512;
                const int64_t n_seqs       = 3;
                lm_ggml_tensor * s   = lm_ggml_new_tensor_4d(ctx, GGML_TYPE_F32, d_state, head_dim, n_head, n_seqs);
                lm_ggml_tensor * x   = lm_ggml_new_tensor_4d(ctx, GGML_TYPE_F32, head_dim, n_head, n_seq_tokens, n_seqs);
                lm_ggml_tensor * dt  = lm_ggml_new_tensor_3d(ctx, GGML_TYPE_F32, n_head, n_seq_tokens, n_seqs);
                lm_ggml_tensor * B   = lm_ggml_new_tensor_4d(ctx, GGML_TYPE_F32, d_state, n_group, n_seq_tokens, n_seqs);
                lm_ggml_tensor * C   = lm_ggml_new_tensor_4d(ctx, GGML_TYPE_F32, d_state, n_group, n_seq_tokens, n_seqs);
                lm_ggml_tensor * ids = lm_ggml_new_tensor_1d(ctx, GGML_TYPE_I32, n_seqs);
        ...
        case LM_GGML_OP_RWKV_WKV6:
            {
                ...
                lm_ggml_tensor  * k = lm_ggml_new_tensor_3d(ctx, GGML_TYPE_F32, S, H, n_tokens);
                lm_ggml_tensor  * v = lm_ggml_new_tensor_3d(ctx, GGML_TYPE_F32, S, H, n_tokens);
                lm_ggml_tensor  * r = lm_ggml_new_tensor_3d(ctx, GGML_TYPE_F32, S, H, n_tokens);
                lm_ggml_tensor  * tf = w;
                lm_ggml_tensor  * td = lm_ggml_new_tensor_3d(ctx, GGML_TYPE_F32, S, H, n_tokens);
                lm_ggml_tensor  * state = lm_ggml_new_tensor_4d(ctx, GGML_TYPE_F32, S, n_seqs, S, H);
                op_tensor = lm_ggml_rwkv_wkv6(ctx, k, v, r, tf, td, state);
            } break;
        case LM_GGML_OP_IM2COL:
            {
                const int n_embd_inp = hparams.n_embd_inp();
                lm_ggml_tensor * b = lm_ggml_new_tensor_4d(ctx, GGML_TYPE_F32, n_embd_inp, w->ne[1], 1, 1);
                op_tensor = lm_ggml_im2col(ctx, w, b, 1, 0, 0, 0, 1, 0, false, GGML_TYPE_F16);
            } break;
                ...
                for (const auto * overrides = ml.tensor_buft_overrides; overrides->pattern != nullptr; ++overrides) {
                    ...
                    if (std::regex_search(tensor_name, pattern)) {
                        ...
                        LLAMA_LOG_DEBUG("tensor %s (%zu MiB %s) buffer type overridden to %s\n",
                                tensor_name.c_str(),
                                lm_ggml_nbytes(t_meta) / 1024 / 1024, ggml_type_name(t_meta->type),
...
std::map<lm_ggml_backend_buffer_type_t, size_t> llama_model::memory_breakdown() const {
    ...
    for (const auto & [ctx, bufs] : pimpl->ctxs_bufs) {
        if (hparams.no_alloc) {
            ...
            GGML_ASSERT(lm_ggml_backend_buffer_get_base(buf) == nullptr);
            ...
            for (const auto & buf : bufs) {
                // GGML_ASSERT(lm_ggml_backend_buffer_get_base(buf.get()) != nullptr); // multi_buffer does not have a defined base
...
lm_ggml_backend_buffer_type_t llama_model::select_buft(int il) const {
    return ::select_buft(
            *pimpl->dev_layer.at(il).buft_list,
            [&](lm_ggml_context * ctx) {
                lm_ggml_tensor * cur = lm_ggml_new_tensor_1d(ctx, GGML_TYPE_F32, hparams.n_embd);
                lm_ggml_tensor * layer_dir = lm_ggml_new_tensor_1d(ctx, GGML_TYPE_F32, hparams.n_embd);
