...
namespace GGUFMeta {
    ...
    bool llama_model_loader::get_arr(const std::string & key, std::vector<T> & result, bool required) {
        ...
        switch (arr_info.gt) {
            ...
            case LM_GGUF_TYPE_INT32:   GGML_ASSERT((std::is_same<T,     int32_t>::value) ||
                                                (std::is_same<T,    uint32_t>::value)); break;
            case LM_GGUF_TYPE_FLOAT32: GGML_ASSERT((std::is_same<T,       float>::value)); break;
            case LM_GGUF_TYPE_STRING:  GGML_ASSERT((std::is_same<T, std::string>::value)); break;
            default:
        ...
        for (const auto & it : weights_map) {
            ...
            if (trace > 0) {
                ...
                LLAMA_LOG_INFO("%s: - tensor split %2d: %32s %-8s [ %s ] %8.2f MiB\n", __func__,
                        sid, lm_ggml_get_name(tensor), ggml_type_name(type), llama_format_tensor_shape(tensor).c_str(),
