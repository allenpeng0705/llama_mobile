...
//
//       struct lm_ggml_tensor * x = lm_ggml_new_tensor_1d(ctx, GGML_TYPE_F32, 1);
//
//       lm_ggml_set_param(ctx, x); // x is an input variable
//
//       struct lm_ggml_tensor * a  = lm_ggml_new_tensor_1d(ctx, GGML_TYPE_F32, 1);
//       struct lm_ggml_tensor * b  = lm_ggml_new_tensor_1d(ctx, GGML_TYPE_F32, 1);
...
//       const int ny = 3;
//
//       struct lm_ggml_tensor * a = lm_ggml_new_tensor_2d(ctx, GGML_TYPE_F32, nx, ny);
//
//       for (int y = 0; y < ny; y++) {
//           for (int x = 0; x < nx; x++) {
...
#define GGML_ASSERT(x) if (!(x)) LM_GGML_ABORT("GGML_ASSERT(%s) failed", #x)
...
extern "C" {
    ...
    LM_GGML_API int64_t ggml_time_us(void);
    ...
    LM_GGML_API int64_t lm_ggml_blck_size(enum ggml_type type);
    LM_GGML_API size_t  ggml_type_size(enum ggml_type type);             // size in bytes for all elements in a block
    LM_GGML_API size_t  lm_ggml_row_size (enum ggml_type type, int64_t ne); // size in bytes for all elements in a row

    LM_GGML_DEPRECATED(
    LM_GGML_API double ggml_type_sizef(enum ggml_type type), // ggml_type_size()/lm_ggml_blck_size() as float
    "use lm_ggml_row_size() instead");

    LM_GGML_API const char * ggml_type_name(enum ggml_type type);
    ...
    LM_GGML_API bool    lm_ggml_is_quantized(enum ggml_type type);

    // TODO: temporary until model loading of ggml examples is refactored
    LM_GGML_API enum ggml_type lm_ggml_ftype_to_ggml_type(enum lm_ggml_ftype ftype);
    ...
    LM_GGML_API bool lm_ggml_validate_row_data(enum ggml_type type, const void * data, size_t nbytes);
...
#define LM_GGML_N_TASKS_MAX (-1)
    ...
    //
    LM_GGML_API void lm_ggml_quantize_init(enum ggml_type type);
    LM_GGML_API void lm_ggml_quantize_free(void);

    // some quantization type cannot be used without an importance matrix
    LM_GGML_API bool lm_ggml_quantize_requires_imatrix(enum ggml_type type);
...
#    endif
    ...
    LM_GGML_API const struct ggml_type_traits * lm_ggml_get_type_traits(enum ggml_type type);
