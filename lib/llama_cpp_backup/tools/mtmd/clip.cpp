...
lm_ggml_tensor * clip_graph::build_inp_raw(int channels) {
    lm_ggml_tensor * inp_raw = lm_ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, img.nx, img.ny, channels);
    ...
    clip_model_loader(const char * fname) : fname(fname) {
        ...
        // tensors
        {
            for (int i = 0; i < n_tensors; ++i) {
                ...
                enum ggml_type type = lm_gguf_get_tensor_type(ctx_gguf.get(), i);
                lm_ggml_tensor * cur = lm_ggml_get_tensor(meta, name);
                size_t tensor_size = lm_ggml_nbytes(cur);
                model_size += tensor_size;
                LOG_DBG("%s: tensor[%d]: n_dims = %d, name = %s, tensor_size=%zu, offset=%zu, shape:[%" PRIu64 ", %" PRIu64 ", %" PRIu64 ", %" PRIu64 "], type = %s\n",
                    __func__, i, lm_ggml_n_dims(cur), cur->name, tensor_size, offset, cur->ne[0], cur->ne[1], cur->ne[2], cur->ne[3], ggml_type_name(type));
            }
        }
    }
...
bool clip_image_batch_encode(clip_ctx * ctx, const int n_threads, const clip_image_f32_batch * imgs_c_ptr, float * vec) {
    ...
    auto set_input_f32 = [&get_inp_tensor](const char * name, std::vector<float> & values) {
        ...
        GGML_ASSERT(lm_ggml_nelements(cur) == (int64_t)values.size());
