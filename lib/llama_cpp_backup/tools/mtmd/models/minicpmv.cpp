...
lm_ggml_cgraph * clip_graph_minicpmv::build() {
    ...
    lm_ggml_tensor * omega = lm_ggml_new_tensor_1d(ctx0, GGML_TYPE_F32, n_embd_proj / 4);
    lm_ggml_set_name(omega, "omega");
    lm_ggml_set_input(omega);

    // 2D input positions (using float for sinusoidal embeddings)
    lm_ggml_tensor * pos_h = lm_ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, 1, n_pos);
    lm_ggml_set_name(pos_h, "pos_h");
    lm_ggml_set_input(pos_h);
    lm_ggml_tensor * pos_w = lm_ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, 1, n_pos);
    lm_ggml_set_name(pos_w, "pos_w");
    lm_ggml_set_input(pos_w);

    // for selecting learned pos embd, used by ViT
    struct lm_ggml_tensor * positions = lm_ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, n_pos);
