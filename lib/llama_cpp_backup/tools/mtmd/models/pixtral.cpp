...
lm_ggml_cgraph * clip_graph_pixtral::build() {
    const int n_merge = hparams.n_merge;

    // 2D input positions
    lm_ggml_tensor * pos_h = lm_ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, n_patches);
    lm_ggml_set_name(pos_h, "pos_h");
    lm_ggml_set_input(pos_h);

    lm_ggml_tensor * pos_w = lm_ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, n_patches);
