...
lm_ggml_cgraph * clip_graph_qwen2vl::build() {
    ...
    lm_ggml_tensor * positions = lm_ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, num_position_ids);
    ...
    if (use_window_attn) {
        ...
        inv_window_idx = lm_ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, n_pos / 4);
        lm_ggml_set_name(inv_window_idx, "inv_window_idx");
        lm_ggml_set_input(inv_window_idx);
        // mask for window attention
        window_mask = lm_ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_pos, n_pos);
        lm_ggml_set_name(window_mask, "window_mask");
        lm_ggml_set_input(window_mask);

        // if flash attn is used, we need to pad the mask and cast to f16
        if (flash_attn_type == CLIP_FLASH_ATTN_TYPE_ENABLED) {
            window_mask = lm_ggml_cast(ctx0, window_mask, GGML_TYPE_F16);
        }
    ...
    if (use_window_attn) {
        window_idx = lm_ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, n_pos / 4);
