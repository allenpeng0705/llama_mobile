...
llm_build_qwen3next::llm_build_qwen3next(const llama_model & model, const llm_graph_params & params) :
    ...
    lm_ggml_tensor * causal_mask =
        lm_ggml_tri(ctx0, lm_ggml_fill_inplace(ctx0, lm_ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, CHUNK_SIZE, CHUNK_SIZE), 1.0f),
                    LM_GGML_TRI_TYPE_LOWER);

    lm_ggml_tensor * identity = lm_ggml_diag(ctx0, lm_ggml_fill_inplace(ctx0, lm_ggml_new_tensor_1d(ctx0, GGML_TYPE_F32, CHUNK_SIZE), 1.0f));
...
lm_ggml_tensor * llm_build_qwen3next::build_layer_attn_linear(
    ...
    GGML_ASSERT(lm_ggml_nelements(attn_out_1d) + lm_ggml_nelements(state_1d) == lm_ggml_nelements(attn_out));
