    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self_attention
        {
            ...
            if (is_mla) {
                ...
                // split into {n_embd_head_qk_nope, n_head, n_tokens}
                lm_ggml_tensor * k_nope =
                    lm_ggml_view_3d(ctx0, kv, n_embd_head_qk_nope, n_head, n_tokens,
                                 lm_ggml_row_size(kv->type, n_embd_head_qk_nope + n_embd_head_v),
                                 lm_ggml_row_size(kv->type, n_embd_head_qk_nope + n_embd_head_v) * n_head, 0);
                cb(k_nope, "k_nope_view", il);

                // and {n_embd_head_v, n_head, n_tokens}
                lm_ggml_tensor * Vcur = lm_ggml_view_3d(ctx0, kv, n_embd_head_v, n_head, n_tokens,
                                                  lm_ggml_row_size(kv->type, n_embd_head_qk_nope + n_embd_head_v),
