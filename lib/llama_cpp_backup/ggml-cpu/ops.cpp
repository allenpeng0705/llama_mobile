...
static void lm_ggml_compute_forward_dup_same_cont(
    ...
    GGML_ASSERT(lm_ggml_nelements(dst) == lm_ggml_nelements(src0));
    GGML_ASSERT(lm_ggml_is_contiguous(dst) && lm_ggml_is_contiguous(src0));
...
static void lm_ggml_compute_forward_dup_flt(
    ...
    GGML_ASSERT(lm_ggml_nelements(dst) == lm_ggml_nelements(src0));
    GGML_ASSERT(!lm_ggml_is_quantized(src0->type) && !lm_ggml_is_quantized(dst->type));
...
static void lm_ggml_compute_forward_dup_to_q(
    ...
    GGML_ASSERT(lm_ggml_nelements(dst) == lm_ggml_nelements(src0));
    GGML_ASSERT(!lm_ggml_is_quantized(src0->type));
...
static void lm_ggml_compute_forward_dup_from_q(
    ...
    GGML_ASSERT((ne10 % qk) == 0 || lm_ggml_is_contiguous(dst));
...
void lm_ggml_compute_forward_dup(
    ...
    switch (src0->type) {
        case GGML_TYPE_F16:
            {
                /**/ if (dst->type == GGML_TYPE_F16)  lm_ggml_compute_forward_dup_flt<lm_ggml_fp16_t, lm_ggml_fp16_t>(params, dst);
                else if (dst->type == GGML_TYPE_BF16) lm_ggml_compute_forward_dup_flt<lm_ggml_fp16_t, lm_ggml_bf16_t>(params, dst);
                else if (dst->type == GGML_TYPE_F32)  lm_ggml_compute_forward_dup_flt<lm_ggml_fp16_t, float      >(params, dst);
                else lm_ggml_compute_forward_dup_to_q<lm_ggml_fp16_t>(params, dst);
            } break;
        case GGML_TYPE_BF16:
            {
                /**/ if (dst->type == GGML_TYPE_F16)  lm_ggml_compute_forward_dup_flt<lm_ggml_bf16_t, lm_ggml_fp16_t>(params, dst);
                else if (dst->type == GGML_TYPE_BF16) lm_ggml_compute_forward_dup_flt<lm_ggml_bf16_t, lm_ggml_bf16_t>(params, dst);
                else if (dst->type == GGML_TYPE_F32)  lm_ggml_compute_forward_dup_flt<lm_ggml_bf16_t, float      >(params, dst);
                else lm_ggml_compute_forward_dup_to_q<lm_ggml_bf16_t>(params, dst);
            } break;
        case GGML_TYPE_F32:
            {
                /**/ if (dst->type == GGML_TYPE_F16)  lm_ggml_compute_forward_dup_flt<float, lm_ggml_fp16_t>(params, dst);
                else if (dst->type == GGML_TYPE_BF16) lm_ggml_compute_forward_dup_flt<float, lm_ggml_bf16_t>(params, dst);
                else if (dst->type == GGML_TYPE_F32)  lm_ggml_compute_forward_dup_flt<float, float      >(params, dst);
                else if (dst->type == GGML_TYPE_I32)  lm_ggml_compute_forward_dup_flt<float, int32_t    >(params, dst);
                else lm_ggml_compute_forward_dup_to_q<float>(params, dst);
            } break;
        case GGML_TYPE_I32:
            {
                if (dst->type == GGML_TYPE_F32) lm_ggml_compute_forward_dup_flt<int32_t, float>(params, dst);
                else LM_GGML_ABORT("not implemented");
            } break;
        default:
            {
                if (lm_ggml_is_quantized(src0->type) && dst->type == GGML_TYPE_F32) {
...
static void lm_ggml_compute_forward_add_q_f32(
    ...
    GGML_ASSERT(lm_ggml_are_same_shape(src0, src1) && lm_ggml_are_same_shape(src0, dst));
...
void lm_ggml_compute_forward_add_id(
    ...
    switch (src0->type) {
        ...
        default:
            {
                LM_GGML_ABORT("unsupported type for lm_ggml_compute_forward_add_id: %s", ggml_type_name(src0->type));
            }
    }
}
...
static void lm_ggml_compute_forward_add1_f32(
    ...
    GGML_ASSERT(lm_ggml_are_same_shape(src0, dst));
    GGML_ASSERT(lm_ggml_is_scalar(src1));
...
static void lm_ggml_compute_forward_add1_f16_f32(
    ...
    GGML_ASSERT( nb0 == sizeof(lm_ggml_fp16_t));
    GGML_ASSERT(nb00 == sizeof(lm_ggml_fp16_t));
...
static void lm_ggml_compute_forward_add1_bf16_f32(
    ...
    GGML_ASSERT( nb0 == sizeof(lm_ggml_bf16_t));
    GGML_ASSERT(nb00 == sizeof(lm_ggml_bf16_t));
...
static void lm_ggml_compute_forward_acc_f32(
    ...
    GGML_ASSERT(offset + (ne10 == 0 ? 0 : ne10-1)*nb0  + (ne11 == 0 ? 0 : ne11-1)*nb1  + (ne12 == 0 ? 0 : ne12-1)*nb2  + (ne13 == 0 ? 0 : ne13-1)*nb3  < lm_ggml_nbytes(dst));
    GGML_ASSERT(offset + (ne10 == 0 ? 0 : ne10-1)*nb00 + (ne11 == 0 ? 0 : ne11-1)*nb01 + (ne12 == 0 ? 0 : ne12-1)*nb02 + (ne13 == 0 ? 0 : ne13-1)*nb03 < lm_ggml_nbytes(src0));
...
static void lm_ggml_compute_forward_count_equal_i32(
    ...
    GGML_ASSERT(lm_ggml_are_same_shape(src0, src1));
    GGML_ASSERT(lm_ggml_is_scalar(dst));
...
static void lm_ggml_compute_forward_repeat_f32(
    ...
    GGML_ASSERT(lm_ggml_can_repeat(src0, dst));
...
static void lm_ggml_compute_forward_repeat_back_f32(
    ...
    GGML_ASSERT(lm_ggml_can_repeat(dst, src0));
...
static void lm_ggml_compute_forward_concat_f16(
    ...
    GGML_ASSERT(ggml_type_size(src0->type) == sizeof(lm_ggml_fp16_t));
...
static void lm_ggml_compute_forward_tri_f32(const lm_ggml_compute_params * params, lm_ggml_tensor * dst) {
    ...
    GGML_ASSERT(lm_ggml_is_contiguous(src0));
...
static void lm_ggml_compute_forward_reglu_f32(
    ...
    GGML_ASSERT(lm_ggml_is_contiguous_1(src0));
    GGML_ASSERT(lm_ggml_is_contiguous_1(dst));

    if (src1) {
        GGML_ASSERT(lm_ggml_is_contiguous_1(src1));
    ...
    GGML_ASSERT(lm_ggml_nrows(dst) == nr);
...
static void lm_ggml_compute_forward_rms_norm_back_f32(
    ...
    GGML_ASSERT(lm_ggml_are_same_shape(src0, dst) && lm_ggml_are_same_shape(src0, src1));
...
static void lm_ggml_compute_forward_scale_f32(
    ...
    GGML_ASSERT(lm_ggml_is_contiguous(src0));
    GGML_ASSERT(lm_ggml_is_contiguous(dst));
    GGML_ASSERT(lm_ggml_are_same_shape(src0, dst));
...
static void lm_ggml_compute_forward_set_f32(
    ...
    GGML_ASSERT(offset + im0*nb0  + im1*nb1  + im2*nb2  + im3*nb3  <= lm_ggml_nbytes(dst));
        ...
        case GGML_TYPE_F32:
            {
                if (src1->type == GGML_TYPE_I64) {
                    ...
                    LM_GGML_ABORT("src1->type = %d (%s) not supported", src1->type, ggml_type_name(src1->type));
                }
            } break;
        default:
            {
                LM_GGML_ABORT("src0->type = %d (%s) not supported", src0->type, ggml_type_name(src0->type));
            }
    }
}
...
static void lm_ggml_compute_forward_get_rows_back_f32_f16(
    ...
    GGML_ASSERT( dst->ne[0] == nc);
    GGML_ASSERT(src0->nb[0] == sizeof(lm_ggml_fp16_t));
...
static void lm_ggml_compute_forward_soft_max_ext_back_f32(
    ...
    GGML_ASSERT(lm_ggml_is_contiguous(src0));
    GGML_ASSERT(lm_ggml_is_contiguous(src1));
    GGML_ASSERT(lm_ggml_is_contiguous(dst));
    GGML_ASSERT(lm_ggml_are_same_shape(src0, dst));
    GGML_ASSERT(lm_ggml_are_same_shape(src1, dst));
...
static void lm_ggml_call_mul_mat(ggml_type type, const lm_ggml_compute_params * params, int64_t m, int64_t n, int64_t k,
                              void * a, void * b, float * c) {
    const ggml_type_traits * traits = lm_ggml_get_type_traits(type);
...
static void lm_ggml_compute_forward_conv_2d_impl(const lm_ggml_compute_params * params,
    ...
    GGML_ASSERT(lm_ggml_is_contiguous(kernel));
    GGML_ASSERT(kernel_type == GGML_TYPE_F16 || kernel_type == GGML_TYPE_F32);
    GGML_ASSERT(kernel->type == kernel_type);

    const ggml_type_traits * traits = lm_ggml_get_type_traits(kernel_type);
    ...
    while (cdata < data_end) {
        ...
        for (int64_t i = 0; i < rs; ++i) {
            ...
            for (int ki = 0; ki < k; ++ki) {
                const float srow_j = (src->type == GGML_TYPE_F32) ? ((const float*)srow)[j] : LM_GGML_CPU_FP16_TO_FP32(((const lm_ggml_fp16_t*)srow)[j]);
                switch (op) {
...
static void lm_ggml_compute_forward_arange_f32(
    ...
    GGML_ASSERT(lm_ggml_nelements(dst) == steps);
...
static void lm_ggml_compute_forward_flash_attn_ext_f16_one_chunk(
    ...
    ggml_type         const k_vec_dot_type = lm_ggml_get_type_traits_cpu(k->type)->vec_dot_type;
...
static void lm_ggml_compute_forward_flash_attn_back_f32(
    ...
    GGML_ASSERT(lm_ggml_blck_size(result_type) == 1);
...
static void lm_ggml_compute_forward_ssm_scan_f32(
    ...
    GGML_ASSERT(lm_ggml_nelements(src1) + nc*nr*nh*ns == lm_ggml_nelements(dst));
    GGML_ASSERT(src0->nb[0] == sizeof(float));
    GGML_ASSERT(src1->nb[0] == sizeof(float));
...
static void lm_ggml_compute_forward_cross_entropy_loss_back_f32(
    ...
    GGML_ASSERT(lm_ggml_is_contiguous(dst));
    GGML_ASSERT(lm_ggml_is_contiguous(src0f));
    GGML_ASSERT(lm_ggml_is_contiguous(src1f));
    GGML_ASSERT(lm_ggml_is_contiguous(grad));
    GGML_ASSERT(lm_ggml_are_same_shape(src0f, src1f) && lm_ggml_are_same_shape(src0f, dst));
...
static void lm_ggml_compute_forward_opt_step_adamw_f32(
    ...
    GGML_ASSERT(lm_ggml_are_same_shape(src0, src0_grad));
    GGML_ASSERT(lm_ggml_are_same_shape(src0, src0_grad_m));
    GGML_ASSERT(lm_ggml_are_same_shape(src0, src0_grad_v));
    GGML_ASSERT(lm_ggml_nelements(adamw_params) == 7);
...
static void lm_ggml_compute_forward_opt_step_sgd_f32(const lm_ggml_compute_params * params, lm_ggml_tensor * dst) {
    ...
    GGML_ASSERT(lm_ggml_are_same_shape(src0, src0_grad));
    GGML_ASSERT(lm_ggml_nelements(sgd_params) == 2);
