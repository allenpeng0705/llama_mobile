...
void lm_ggml_backend_cpu_set_n_threads(lm_ggml_backend_t backend_cpu, int n_threads) {
    GGML_ASSERT(lm_ggml_backend_is_cpu(backend_cpu));
...
static bool lm_ggml_backend_cpu_device_supports_op(lm_ggml_backend_dev_t dev, const struct lm_ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case LM_GGML_OP_MUL_MAT:
            return src1->type == GGML_TYPE_F32 || src1->type == lm_ggml_get_type_traits_cpu(src0->type)->vec_dot_type;
        ...
        case LM_GGML_OP_OUT_PROD:
            return (src0->type == GGML_TYPE_F32 || (lm_ggml_is_quantized(src0->type) && src0->ne[2] == src1->ne[2] && src0->ne[3] == src1->ne[3])) &&
