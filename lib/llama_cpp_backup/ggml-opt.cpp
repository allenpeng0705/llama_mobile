...
void lm_ggml_opt_dataset_get_batch(lm_ggml_opt_dataset_t dataset, struct lm_ggml_tensor * data_batch, struct lm_ggml_tensor * labels_batch, int64_t ibatch) {
    GGML_ASSERT(   data_batch && lm_ggml_is_contiguous(data_batch));
    GGML_ASSERT(!labels_batch || lm_ggml_is_contiguous(labels_batch));
...
static lm_ggml_cgraph * dup_graph(lm_ggml_context * ctx, lm_ggml_cgraph * src) {
    ...
    for (int i = 0; i < src->n_nodes; ++i) {
        ...
        GGML_ASSERT(igrad_src != LM_GGML_HASHSET_FULL);
        GGML_ASSERT(lm_ggml_bitset_get(src->visited_hash_set.used, igrad_src));
        GGML_ASSERT(igrad_dst != LM_GGML_HASHSET_FULL);
        GGML_ASSERT(lm_ggml_bitset_get(dst->visited_hash_set.used, igrad_dst));
...
static void lm_ggml_opt_build(lm_ggml_opt_context_t opt_ctx) {
    GGML_ASSERT(opt_ctx->ctx_compute && "no compute context set, either use static graphs or set one with lm_ggml_opt_prepare_alloc");
    ...
    for (int i = 0; i < opt_ctx->gf->n_nodes; ++i) {
        ...
        GGML_ASSERT(!(node->flags & LM_GGML_TENSOR_FLAG_LOSS) && "support for extra loss terms not implemented");
    }
    ...
    if (opt_ctx->grad_accs.empty()) {
        GGML_ASSERT(opt_ctx->build_type_alloc >= LM_GGML_OPT_BUILD_TYPE_GRAD);
        ...
        for (int i = 0; i < n_nodes; ++i) {
            ...
            if ((accumulate && (node->flags & LM_GGML_TENSOR_FLAG_PARAM)) || (node->flags & LM_GGML_TENSOR_FLAG_LOSS)) {
                opt_ctx->grad_accs[i] = lm_ggml_new_tensor(opt_ctx->ctx_static, GGML_TYPE_F32, LM_GGML_MAX_DIMS, node->ne);
            } else {
        ...
        if (need_momenta && opt_ctx->build_type_alloc >= LM_GGML_OPT_BUILD_TYPE_OPT) {
            ...
            for (int i = 0; i < n_nodes; ++i) {
                lm_ggml_tensor * node = opt_ctx->gf->nodes[i];
                if (node->flags & LM_GGML_TENSOR_FLAG_PARAM) {
                    opt_ctx->grad_m[i] = lm_ggml_new_tensor(opt_ctx->ctx_static, GGML_TYPE_F32, LM_GGML_MAX_DIMS, node->ne);
                    opt_ctx->grad_v[i] = lm_ggml_new_tensor(opt_ctx->ctx_static, GGML_TYPE_F32, LM_GGML_MAX_DIMS, node->ne);
                } else {
    ...
    GGML_ASSERT(opt_ctx->build_type_alloc == LM_GGML_OPT_BUILD_TYPE_OPT);

    // gb_opt == graph backward optimize, forward pass, then backward pass to calculate gradients, then optimizer step.
    opt_ctx->gb_opt = lm_ggml_graph_dup(opt_ctx->ctx_compute, opt_ctx->gb_grad, /*force_grads =*/ true);

    opt_ctx->opt_step_params = lm_ggml_new_tensor_1d(opt_ctx->ctx_cpu, GGML_TYPE_F32, need_momenta ? 7 : 2);
...
void lm_ggml_opt_eval(lm_ggml_opt_context_t opt_ctx, lm_ggml_opt_result_t result) {
    ...
    GGML_ASSERT(lm_ggml_is_scalar(opt_ctx->loss));
    ...
    GGML_ASSERT(lm_ggml_is_scalar(opt_ctx->ncorrect));
...
void lm_ggml_opt_epoch(
    ...
    GGML_ASSERT(lm_ggml_opt_static_graphs(opt_ctx) && "lm_ggml_opt_epoch requires static graphs");
