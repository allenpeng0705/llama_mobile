...
static int lm_ggml_metal_op_encode_impl(lm_ggml_metal_op_t ctx, int idx) {
    ...
    switch (node->op) {
        ...
        case LM_GGML_OP_FILL:
            {
                n_fuse = lm_ggml_metal_op_fill(ctx, idx);
            } break;
...
int lm_ggml_metal_op_acc(lm_ggml_metal_op_t ctx, int idx) {
    ...
    GGML_ASSERT(lm_ggml_is_contiguous(op->src[0]));
    GGML_ASSERT(lm_ggml_is_contiguous(op->src[1]));
    ...
    auto pipeline = lm_ggml_metal_library_get_pipeline_bin(lib, LM_GGML_OP_ADD, 1, false);
...
int lm_ggml_metal_op_fill(lm_ggml_metal_op_t ctx, int idx) {
    ...
    const float val = lm_ggml_get_op_params_f32(op, 0);

    lm_ggml_metal_kargs_fill args = {
        /*.val =*/ val
    };
...
int lm_ggml_metal_op_glu(lm_ggml_metal_op_t ctx, int idx) {
    ...
    if (op->src[1]) {
        GGML_ASSERT(lm_ggml_are_same_shape(op->src[0], op->src[1]));
    }
...
int lm_ggml_metal_op_cumsum(lm_ggml_metal_op_t ctx, int idx) {
    ...
    GGML_ASSERT(lm_ggml_is_contiguous_rows(op->src[0]));
...
int lm_ggml_metal_op_ssm_scan(lm_ggml_metal_op_t ctx, int idx) {
    ...
    GGML_ASSERT(d_state <= lm_ggml_metal_pipeline_max_theads_per_threadgroup(pipeline));
...
int lm_ggml_metal_op_cpy(lm_ggml_metal_op_t ctx, int idx) {
    ...
    GGML_ASSERT(ne00 % lm_ggml_blck_size(op->src[0]->type) == 0);
...
int lm_ggml_metal_op_mul_mat_id(lm_ggml_metal_op_t ctx, int idx) {
    ...
    GGML_ASSERT(!lm_ggml_is_transposed(op->src[0]));
    GGML_ASSERT(!lm_ggml_is_transposed(op->src[1]));
    ...
    if (props_dev->has_simdgroup_mm && ne00 >= 64 && (ne21 >= ne21_mm_id_min)) {
...

        {
            ...
            GGML_ASSERT(ne02 <= lm_ggml_metal_pipeline_max_theads_per_threadgroup(pipeline));
...
size_t lm_ggml_metal_op_flash_attn_ext_extra_blk(const lm_ggml_tensor * op) {
  ...
  //LM_GGML_TENSOR_LOCALS(uint64_t, nb2, op->src[2], nb);
    ...
    res += LM_GGML_PAD(ggml_type_size(GGML_TYPE_I8)*ne0*ne1*ne32*ne33, 32);
...
int lm_ggml_metal_op_flash_attn_ext(lm_ggml_metal_op_t ctx, int idx) {
    ...
    //GGML_ASSERT(lm_ggml_are_same_shape (src1, src2));
...
#define FATTN_SMEM(nsg) (LM_GGML_PAD((nqptg*(LM_GGML_PAD(ne00, 128) + 4*ncpsg*(nsg)) + 2*LM_GGML_PAD(ne20, 128)*(nsg))*(sizeof(float)/2), 16))
        ...
        GGML_ASSERT(nsg*32 <= lm_ggml_metal_pipeline_max_theads_per_threadgroup(pipeline));
...
int lm_ggml_metal_op_bin(lm_ggml_metal_op_t ctx, int idx) {
    ...
    GGML_ASSERT(lm_ggml_is_contiguous_rows(op->src[0]));
    GGML_ASSERT(lm_ggml_is_contiguous_rows(op->src[1]));
...
int lm_ggml_metal_op_im2col(lm_ggml_metal_op_t ctx, int idx) {
    ...
    GGML_ASSERT(KH*KW <= lm_ggml_metal_pipeline_max_theads_per_threadgroup(pipeline));
