...
struct lm_gguf_kv {
    ...
    const T & get_val(const size_t i = 0) const {
        GGML_ASSERT(type_to_lm_gguf_type<T>::value == type);
...
struct lm_gguf_context * lm_gguf_init_from_file_impl(FILE * file, struct lm_gguf_init_params params) {
    ...
    if (params.ctx != nullptr) {
        ...
        *params.ctx = lm_ggml_init(pdata);
        ...
        if (!params.no_alloc) {
            data = lm_ggml_new_tensor_1d(ctx_data, GGML_TYPE_I8, ctx->size);
...
const char * lm_gguf_get_key(const struct lm_gguf_context * ctx, int64_t key_id) {
    GGML_ASSERT(key_id >= 0 && key_id < lm_gguf_get_n_kv(ctx));
...
const void * lm_gguf_get_arr_data(const struct lm_gguf_context * ctx, int64_t key_id) {
    GGML_ASSERT(key_id >= 0 && key_id < lm_gguf_get_n_kv(ctx));
    GGML_ASSERT(ctx->kv[key_id].get_type() != LM_GGUF_TYPE_STRING);
...
size_t lm_gguf_get_tensor_offset(const struct lm_gguf_context * ctx, int64_t tensor_id) {
    GGML_ASSERT(tensor_id >= 0 && tensor_id < lm_gguf_get_n_tensors(ctx));
...
enum ggml_type lm_gguf_get_tensor_type(const struct lm_gguf_context * ctx, int64_t tensor_id) {
    GGML_ASSERT(tensor_id >= 0 && tensor_id < lm_gguf_get_n_tensors(ctx));
...
static void lm_gguf_check_reserved_keys(const std::string & key, const T val) {
    if (key == LM_GGUF_KEY_GENERAL_ALIGNMENT) {
        if constexpr (std::is_same<T, uint32_t>::value) {
            GGML_ASSERT(val > 0 && (val & (val - 1)) == 0 && LM_GGUF_KEY_GENERAL_ALIGNMENT " must be power of 2");
        } else {
            LM_GGML_UNUSED(val);
...
void lm_gguf_set_tensor_type(struct lm_gguf_context * ctx, const char * name, enum ggml_type type) {
...
struct lm_gguf_writer_base {
    ...
    virtual void write_tensor_data(const struct lm_gguf_tensor_info & info, size_t offset_data, size_t alignment) = 0;
    ...
    void write(const enum lm_gguf_type & val) {
        write(int32_t(val));
    }

    void write(const struct lm_gguf_kv & kv) {
    ...
    void write_tensor_meta(const struct lm_gguf_tensor_info & info) {
        write(info.t.name);
...
struct lm_gguf_writer_buf final : public lm_gguf_writer_base {
    std::vector<int8_t> & buf;

    lm_gguf_writer_buf(std::vector<int8_t> & buf) : buf(buf) {}

    using lm_gguf_writer_base::write;
    ...
    void write_tensor_data(const struct lm_gguf_tensor_info & info, const size_t offset_data, const size_t alignment) override {
        GGML_ASSERT(buf.size() - offset_data == info.offset);

        GGML_ASSERT(lm_ggml_is_contiguous(&info.t));
...
// file based writer
struct lm_gguf_writer_file final : public lm_gguf_writer_base {
    FILE * file;

    lm_gguf_writer_file(FILE* file) : file(file) {}
    ...
    void write_tensor_data(const struct lm_gguf_tensor_info & info, const size_t offset_data, const size_t alignment) override {
        ...
        if (info.t.buffer) {
            lm_ggml_backend_tensor_get(&info.t, buf.data(), 0, nbytes);
        } else {
            GGML_ASSERT(info.t.data);
...
static void lm_gguf_write_out(const struct lm_gguf_context * ctx, writer_t & gw, bool only_meta) {
    const int64_t n_kv      = lm_gguf_get_n_kv(ctx);
    const int64_t n_tensors = lm_gguf_get_n_tensors(ctx);
