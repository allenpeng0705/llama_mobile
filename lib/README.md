...
output/
├── include/
│   ├── llama_mobile_api.h   # Main API header file (standard implementation)
│   └── llama_mobile_mnn.h   # MNN-specific API header file
├── lib/
...
## MNN API Documentation (llama_mobile_mnn.h)

The `llama_mobile_mnn.h` header provides a C++ API for integrating MNN (Mobile Neural Network) engine support into the llama_mobile library. This API is designed for mobile and embedded devices, offering optimized performance through MNN's lightweight inference engine.

### Core Structures

#### mnn_mobile_context

The main context class that encapsulates all model state and functionality:

```cpp
struct mnn_mobile_context {
    // Prediction state
    bool is_predicting;            // Whether the model is currently generating text
    bool is_interrupted;           // Whether generation has been interrupted
    bool has_next_token;           // Whether there's a next token available
    ...
    // Token counters
    size_t num_prompt_tokens;      // Number of tokens in the current prompt
    size_t num_tokens_predicted;   // Number of tokens generated in current completion
    size_t n_past;                 // Number of tokens processed so far
    size_t n_remain;               // Number of tokens remaining to generate
    
    // Model and context pointers
    std::shared_ptr<MNN::Interpreter> interpreter; // MNN interpreter
    MNN::Session* session;                         // MNN session
    std::shared_ptr<MNN::Transformer::Llm> llm;    // MNN LLM transformer
    std::shared_ptr<MNN::Transformer::Embedding> embedding_model; // Embedding model
    bool model_loaded;                             // Whether model is loaded
    float loading_progress;                        // Model loading progress (0.0-1.0)
    bool is_load_interrupted;                      // Whether loading was interrupted
    ...
    // Stopping conditions
    bool truncated;                // Whether the output was truncated
    bool stopped_eos;              // Whether generation stopped due to EOS token
    bool stopped_word;             // Whether generation stopped due to stop word
    bool stopped_limit;            // Whether generation stopped due to token limit
    std::string stopping_word;     // The stop word that triggered stopping
    bool incomplete;               // Whether the generation was incomplete
    ...
    bool conversation_active;      // Whether a conversation is active
    ...
    bool use_metal;                // Whether to use Metal for iOS
    bool use_neon;                 // Whether to use Neon for Android
...
struct completion_token_output
{
    ...
    std::string token_str;         ///< String representation of the selected token
};
```
...
4. Store the string representation of the selected token for easy access

#### Multimodal Support Structures

The following structures enable multimodal capabilities (text + images + audio) in the MNN API:
...
{
    void* image_data;    ///< Raw image data buffer
...
This structure represents an image part of a multimodal prompt, containing the raw image data and its dimensions.
...
{
    const char* file_path;  ///< Path to audio file (alternative to waveform data)
    float* waveform;        ///< Raw audio waveform data (alternative to file path)
    size_t waveform_size;   ///< Size of waveform data in samples
};
```

This structure represents an audio part of a multimodal prompt, supporting either file path or raw waveform data.
...
{
    const char* prompt_template;  ///< Template string for formatting the prompt
    prompt_image_part* images;    ///< Array of images to include in the prompt
    size_t image_count;           ///< Number of images in the array
    prompt_audio_part* audios;    ///< Array of audio parts to include in the prompt
    size_t audio_count;           ///< Number of audio parts in the array
};
```

This structure encapsulates a complete multimodal prompt, combining text (via the template), images, and audio. It is used with functions like `generateMultimodalResponse()` and `tokenizeMultimodal()` to process prompts with multiple media types.
...
// LoRA adapter update strategies
...
```cpp
// Begin a new completion generation process
void beginCompletion();

// Generate the next token in the completion
completion_token_output nextToken();

// End the current completion generation process
void endCompletion();
```
...
// Continue a conversation with detailed timing information
...
```cpp
// Generate a response to a multimodal prompt (text, images, audio)
...
int main() {
    // Create a new MNN context
    auto context = std::make_unique<llama_mobile::mnn_mobile_context>();
    
    // Load the MNN model
    if (!context->loadModel("/path/to/model.mnn", 2048, 4, true, false)) {
        std::cerr << "Failed to load model" << std::endl;
        return 1;
    }
    
    // Set generation parameters
    context->setGenerationParams(0.8f, 0.95f, 40.0f, 1.0f);
    
    // Generate a completion
    std::string prompt = "Hello, how are you today?";
    std::string response = context->generateResponse(prompt, 100);
    
    std::cout << "Generated response: " << response << std::endl;
    
    // Clean up (context is automatically destroyed by unique_ptr)
    return 0;
}
```
...
int main() {
    ...
    if (!context->loadModel("/path/to/model.mnn", 2048, 4)) {
        std::cerr << "Failed to load model" << std::endl;
        return 1;
    }
    
    // Start a conversation
    std::string user_input = "What is machine learning?";
    std::string response = context->generateResponse(user_input, 150);
    std::cout << "Bot: " << response << std::endl;
    
    // Continue the conversation (history is maintained automatically)
    user_input = "Can you explain it in simpler terms?";
    response = context->generateResponse(user_input, 150);
    std::cout << "Bot: " << response << std::endl;
    
    // Check conversation status
    std::cout << "Conversation active: " << (context->isConversationActive() ? "yes" : "no") << std::endl;
...
int main() {
    ...
    // Load both the main model and embedding model
    if (!context->loadModel("/path/to/model.mnn", 1024, 4)) {
        std::cerr << "Failed to load main model" << std::endl;
        return 1;
    }
    
    if (!context->loadEmbeddingModel("/path/to/embedding_config.json", 4)) {
        std::cerr << "Failed to load embedding model" << std::endl;
        return 1;
    }
    
    // Generate embeddings for two sentences
    auto embedding1 = context->generateEmbedding("The quick brown fox jumps over the lazy dog");
    auto embedding2 = context->generateEmbedding("A fast brown fox leaps over a sleepy dog");
    auto embedding3 = context->generateEmbedding("The weather is nice today");
    
    // Calculate similarities
    float similarity12 = context->calculateCosineSimilarity(embedding1, embedding2);
    float similarity13 = context->calculateCosineSimilarity(embedding1, embedding3);
    
    std::cout << "Similarity between sentence 1 and 2: " << similarity12 << std::endl;
    std::cout << "Similarity between sentence 1 and 3: " << similarity13 << std::endl;
    
    // Free embedding data (if needed)
    delete[] embedding1.data;
    delete[] embedding2.data;
    delete[] embedding3.data;
    
    return 0;
}
```

### Text-to-Speech with BERT-VITS2
...
int main() {
    ...
    // Initialize TTS (BERT-VITS2)
    if (!context->initTTS("/path/to/bertvits2_config", llama_mobile::TTS_BERTVITS2)) {
    ...
    if (!context->generateAudioFromText(text, "/path/to/output.wav")) {
        std::cerr << "Failed to generate audio" << std::endl;
        return 1;
    }
    
    std::cout << "Audio generated successfully!" << std::endl;
    
    // Alternative: Generate raw audio waveform
    int sample_rate;
    std::vector<float> audio_data;
    if (context->generateAudioWaveform("Another test", sample_rate, audio_data)) {
        std::cout << "Raw audio generated: " << audio_data.size() << " samples at " << sample_rate << " Hz" << std::endl;
    }
...
int main() {
    ...
    // Load base model
    if (!context->loadModel("/path/to/base_model.mnn", 2048, 4)) {
        std::cerr << "Failed to load base model" << std::endl;
        return 1;
    }
    
    // Configure and apply LoRA adapter
    llama_mobile::lora_adapter_t adapter = {
        .name = "my_lora_adapter",
        .path = "/path/to/lora_adapter.mnn",
        .r = 8.0f,
        .alpha = 16.0f,
        .dropout = 0.1f,
        .freeze = false,
        .update_strategy = llama_mobile::ADAPTER_UPDATE_ALL,
        .layers = "all"
    };
    
    if (context->applyLoraAdapter(adapter)) {
        std::cout << "LoRA adapter applied successfully" << std::endl;
    } else {
        std::cerr << "Failed to apply LoRA adapter" << std::endl;
    }
    
    // Generate text using the adapted model
    std::string response = context->generateResponse("What's special about this LoRA adapter?", 100);
    std::cout << "Response with LoRA: " << response << std::endl;
