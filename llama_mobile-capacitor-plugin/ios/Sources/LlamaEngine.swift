...
public final class LlamaEngine: @unchecked Sendable {
    ...
    /// Resolves a possibly bundle-relative path to an absolute filesystem path.
    /// Relative paths (e.g. "models/model.gguf") are resolved against the app
    /// bundle's resource directory, matching how model/vocoder/mmproj/media
    /// resources are packaged into the bundle. Absolute paths pass through
    /// untouched. llama.cpp needs a real path — it cannot open bundle-relative
    /// names from the process working directory.
    private static func resolvedPath(_ p: String) -> String {
        if p.hasPrefix("/") { return p }
        guard let base = Bundle.main.resourceURL else { return p }
        return base.appendingPathComponent(p).path
    }
    ...
    private func start(_ config: LlamaModelConfig) throws {
        ...
        let buf = CBuf()
        defer { buf.release() }
        cc.pointee.model_path = buf.str(Self.resolvedPath(config.modelPath))
    ...
    public func initMultimodal(mmprojPath: String) throws {
        guard let c = currentContext() else { throw LlamaError.notInitialized }
        let status = llama_mobile_multimodal_init(c, Self.resolvedPath(mmprojPath))
        guard status == LLAMA_MOBILE_OK else { throw LlamaError(status: status) }
    }
    ...
    public func ttsInit(vocoderModelPath: String) throws {
        guard let c = currentContext() else { throw LlamaError.notInitialized }
        let status = llama_mobile_tts_init(c, Self.resolvedPath(vocoderModelPath))
        guard status == LLAMA_MOBILE_OK else { throw LlamaError(status: status) }
    }
    ...
    private func buildParams(_ request: LlamaGenerationRequest) -> BuiltParams {
        ...
        if !request.media.isEmpty {
            ...
            for (i, m) in request.media.enumerated() {
                ...
                med[i].path = buf.strOpt(
                    m.kind == .path ? Self.resolvedPath(m.path ?? "") : m.path)
