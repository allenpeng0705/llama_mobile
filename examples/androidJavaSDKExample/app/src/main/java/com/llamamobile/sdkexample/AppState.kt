...
class AppState {
    ...
    fun loadModel(callback: (Boolean) -> Unit) {
        ...
        try {
            ...
            Log.i(TAG, "- **Final Embedding Flag**: $enableEmbedding")

            val params = LlamaMobile.InitParams(
                safeModelPath,                     // modelPath
                safeNCtx,                          // nCtx
                null,                              // chatTemplate (Java SDK order: chatTemplate before systemPrompt)
                safeSystemPrompt.takeIf { it.isNotEmpty() },  // systemPrompt
                512,                              // nBatch
                512,                              // nUbatch
                safeNGpuLayers,                   // nGpuLayers
                safeNThreads,                    // nThreads
                true,                            // useMmap
                false,                           // useMlock
            ...
            Log.i(TAG, "- InitParams embedding enabled: ${params.isEmbedding()}")
            ...
            try {
                ...
                Log.i(TAG, "- Embedding enabled: $enableEmbedding")
