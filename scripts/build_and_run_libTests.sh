...
echo -e "${BLUE}=== llama_mobile Build and Run Tests Script ===${NC}"
...
# Build the library using build_lib.sh
build_library() {
    echo -e "${BLUE}Building llama_mobile library...${NC}"
    
    if [ ! -x "$SCRIPTS_DIR/build_lib.sh" ]; then
        chmod +x "$SCRIPTS_DIR/build_lib.sh"
    fi
    
    "$SCRIPTS_DIR/build_lib.sh"
    if [ $? -ne 0 ]; then
