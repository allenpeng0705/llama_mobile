...
public class LlamaMobile {
    ...
    public enum GrammarName: String, CaseIterable {
        case arithmetic
        case c
        case chess
        case english
    ...
    /// - Parameter name: The name of the grammar file without extension
    /// - Returns: The content of the grammar file, or nil if not found
    public func grammarContent(for name: GrammarName) -> String? {
        // Try multiple bundle loading strategies to ensure we find the framework bundle
        var bundle: Bundle?
        
        // Strategy 1: Use class from this file to find bundle
        bundle = Bundle(for: LlamaMobile.self)
        print("[DEBUG] Strategy 1 - Bundle for LlamaMobile.self: \(bundle?.bundlePath ?? "nil") (id: \(bundle?.bundleIdentifier ?? "nil"))")
        
        // Strategy 2: Try to find framework bundle by identifier
        if bundle?.bundlePath.contains("app") == true {
            print("[DEBUG] Bundle resolved to app bundle, trying framework bundle by identifier")
            bundle = Bundle(identifier: "com.llamamobile")
            print("[DEBUG] Strategy 2 - Bundle by identifier 'com.llamamobile': \(bundle?.bundlePath ?? "nil") (id: \(bundle?.bundleIdentifier ?? "nil"))")
        }
        
        // Strategy 3: Try to find framework bundle by URL (scanning app frameworks)
        if bundle?.bundlePath.contains("app") == true || bundle == nil {
            let appBundle = Bundle.main
            if let appBundlePath = appBundle.resourcePath {
                let appFrameworksPath = appBundlePath + "/Frameworks"
                print("[DEBUG] Strategy 3 - Scanning app frameworks at: \(appFrameworksPath)")
                let fileManager = FileManager.default
                if let frameworkPaths = try? fileManager.contentsOfDirectory(atPath: appFrameworksPath) {
                    print("[DEBUG] Found frameworks in app: \(frameworkPaths)")
                    for frameworkPath in frameworkPaths {
                        if frameworkPath.contains("llama_mobile") {
                            let fullFrameworkPath = appFrameworksPath + "/" + frameworkPath
                            print("[DEBUG] Found llama_mobile framework at: \(fullFrameworkPath)")
                            if let frameworkBundle = Bundle(path: fullFrameworkPath) {
                                bundle = frameworkBundle
                                print("[DEBUG] Strategy 3 - Loaded framework bundle: \(bundle?.bundlePath ?? "nil") (id: \(bundle?.bundleIdentifier ?? "nil"))")
                                break
                            }
                        }
                    }
        ...
        // Strategy 4: Fallback to main bundle if framework bundle not found
        if bundle == nil {
            print("[DEBUG] Framework bundle not found, falling back to main bundle")
            bundle = Bundle.main
            print("[DEBUG] Strategy 4 - Main bundle: \(bundle?.bundlePath ?? "nil") (id: \(bundle?.bundleIdentifier ?? "nil"))")
        }
        
        print("[DEBUG] Final bundle path: \(bundle?.bundlePath ?? "nil")")
        print("[DEBUG] Final bundle identifier: \(bundle?.bundleIdentifier ?? "nil")")
        print("[DEBUG] Final bundle resources path: \(bundle?.resourcePath ?? "nil")")
        
        // Check what's in the bundle's resources directory
        if let resourcesPath = bundle?.resourcePath {
            let fileManager = FileManager.default
            if let resourceContents = try? fileManager.contentsOfDirectory(atPath: resourcesPath) {
                print("[DEBUG] Resources directory contents: \(resourceContents)")
                // Check if grammars directory exists
                if resourceContents.contains("grammars") {
                    let grammarsPath = resourcesPath + "/grammars"
                    if let grammarFiles = try? fileManager.contentsOfDirectory(atPath: grammarsPath) {
                        print("[DEBUG] Grammars directory contents: \(grammarFiles)")
                    }
                }
            }
        }
        
        print("[DEBUG] Looking for grammar: \(name.rawValue).gbnf in subdirectory: grammars")
        
        guard let bundle = bundle else {
            print("[DEBUG] No bundle available")
            return nil
        }
        
        if let urls = try? bundle.urls(forResourcesWithExtension: "gbnf", subdirectory: "grammars") {
            print("[DEBUG] Found gbnf files: \(urls.map { $0.lastPathComponent })")
        }
    ...
    /// - Parameter token: The generated token string, or nil to indicate end of generation
    ...
    public func initialize(with params: InitParams) -> Bool {
        ...
        return params.modelPath.withCString { modelPathPtr in
            ...
            // Handle optional chat template and system prompt
            if let chatTemplate = params.chatTemplate {
                chatTemplate.withCString { chatTemplatePtr in
                    ...
                    if let systemPrompt = params.systemPrompt {
                        systemPrompt.withCString { systemPromptPtr in
                            cParams.system_prompt = systemPromptPtr
    ...
    public func completion(with params: CompletionParams) -> CompletionResult? {
        ...
        // Use withCString to properly manage C string lifetime for prompt
        return params.prompt.withCString { promptPtr -> CompletionResult? in
            ...
            // Set up token callback - temporarily disabled to avoid closure capture issues
            // This may be revisited in future versions with a proper context management solution
            cParams.token_callback = nil
            
            // Set up grammar - use json.gbnf by default if no grammar is specified
            if let grammar = params.grammar ?? grammarContent(for: .json) {
                print("[DEBUG] Using grammar: \(grammar.prefix(100))...")
                return grammar.withCString { grammarPtr -> CompletionResult? in
                    cParams.grammar = grammarPtr
                    return performCompletion(handle: handle, params: &cParams)
                }
            } else {
                print("[DEBUG] No grammar available")
                cParams.grammar = nil
                return performCompletion(handle: handle, params: &cParams)
            }
        }
    ...
    /// Helper method to perform completion with given parameters
    private func performCompletion(handle: llama_mobile_context_handle_t, params: inout llama_mobile_completion_params_c_t) -> CompletionResult? {
        // Perform completion
        var cResult = llama_mobile_completion_result_c_t()
        let status = llama_mobile_completion_c(handle, &params, &cResult)
    ...
    /// Initializes the multimodal projection for vision models
    /// 
    /// This function initializes the multimodal projection layer that allows the model to process images.
    /// You need to provide a path to a projection file (.mmproj) that matches your model.
    /// 
    /// - Parameter projectionPath: Path to the multimodal projection file (.mmproj)
    /// - Parameter useGPU: Whether to use GPU acceleration for image processing
    /// - Returns: `true` if initialization succeeded, `false` otherwise
    /// 
    /// Example:
    /// ```swift
    /// let projectionPath = "/path/to/model.mmproj"
    /// if llamaMobile.initMultimodal(projectionPath: projectionPath) {
    ///     print("Multimodal projection initialized successfully")
    /// }
    /// ```
    public func initMultimodal(projectionPath: String, useGPU: Bool = false) -> Bool {
        guard let handle = contextHandle else {
            return false
        }
        
        let status = projectionPath.withCString { pathPtr in
    ...
    public func multimodalCompletion(with params: CompletionParams, mediaPaths: [String]) -> CompletionResult? {
        ...
        return params.prompt.withCString { promptPtr in
            ...
            // Set up grammar - use json.gbnf by default if no grammar is specified
            if let grammar = params.grammar ?? grammarContent(for: .json) {
                print("[DEBUG] Using grammar in multimodal completion: \(grammar.prefix(100))...")
